...
const GenerateImages = () => {
    ...
    const imageStyle = ['Realistic Style','Realistic', 'Ghibli Style', '3D Style', 'Anime Style', 'Cartoon Style', 'Fantasy Style', 'Portrait Style']
    ...
    <div className='h-full overflow-y-scroll flex p-6 items-start flex-wrap gap-4 text-slate-700'>
      ...
      <form onSubmit={onSubmitHandler} className='w-full max-w-lg bg-white p-4 rounded-lg border border-gray-200'> 
        ...
        <div className='mt-3 flex gap-3 flex-wrap sm:max-w-9/11'>
          {imageStyle.map((item)=> (
            <span onClick={()=> setSelectedStyle(item)} key={item} className={`text-sm px-4 py-1 border rounded-full cursor-pointer
              ${selectedStyle === item ? 'bg-green-50 text-green-700' : 'text-gray-500 border-gray-300'}`}>{item}</span>
          ))}
        </div>
        ...
        <button className=' w-full justify-center flex items-center gap-2 bg-gradient-to-r from-[#00AD25] to-[#04FF50] text-white px-4 py-2 mt-6 
